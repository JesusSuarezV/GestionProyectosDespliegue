...
public class APUControlador {
	...
	public String guardarAPU(@RequestParam("nombre") String nombre, @RequestParam("unidad") String unidad,
		...
		if (servicio.nombreExistenteYVisible(nombre)) {

			model.addAttribute("alertaAPUExistente", "El APU ya existe.");
			return "APU/Crear_APU";
		}
	...
	public String actualizarAPU(@PathVariable int id, @RequestParam("nombre") String nombre,
			@RequestParam("unidad") String unidad, Model model) {

		APU apuExistente = servicio.obtenerAPUPorId(id); // se obtiene el apu

		if (!apuExistente.getNombre().equals(nombre) && servicio.nombreExistenteYVisible(nombre)) {
			model.addAttribute("alertaAPUExistente", "El nombre del APU que se le quiere asigar ya existe para otro APU.");
			model.addAttribute("apu", servicio.obtenerAPUPorId(id));
			return "APU/Editar_APU";
		}
